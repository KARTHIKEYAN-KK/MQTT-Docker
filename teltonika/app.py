...
HOST = "0.0.0.0" #socket.gethostbyname(socket.gethostname())  #function may not work in Linux systems, change to string with IP adress example: "192.168.0.1"
PORT = 5010  #change this to your port
...
def start_server_tigger():
	...
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
		...
		while True:
			...
			conn, addr = s.accept()
			conn.settimeout(30) #connection timeout, change this value to close the socket if no DATA is received for X amount of seconds
			with conn:
...
#	io_dict_raw["_rec_delay_"] = record_delay_counter(timestamp)
	...
	while data_field_position < (2*data_field_length-6):				
		...
		n_string = str({io_dict[int(key, 16)]})
		data = n_string[6:-2]
		result = []
		print("DATA :", data)
		counter = 1  # Counter to keep track of the iterations
